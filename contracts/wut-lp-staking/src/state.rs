...
pub struct Config {
    pub wut_token: CanonicalAddr,
    pub wut_lp_token: CanonicalAddr,
...
impl Config {
    pub fn as_res(&self, api: &dyn Api) -> Result<ConfigResponse, ContractError> {
        let res = ConfigResponse {
            wut_token: api.addr_humanize(&self.wut_token)?.to_string(),
            wut_lp_token: api.addr_humanize(&self.wut_lp_token)?.to_string(),
