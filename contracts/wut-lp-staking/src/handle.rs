...
pub fn bond_hook(deps: DepsMut, env: Env, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    let lp_token: Addr = deps.api.addr_humanize(&config.wut_lp_token)?;
...
pub fn unbond(
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.wut_lp_token)?.to_string(),
...
pub fn claim_rewards(
    ...
    Ok(Response::new()
        .add_messages(vec![CosmosMsg::Wasm(WasmMsg::Execute {
            contract_addr: deps.api.addr_humanize(&config.wut_token)?.to_string(),
            msg: to_binary(&Cw20ExecuteMsg::Transfer {
                recipient: info.sender.to_string(),
                amount: amount.into(),
            })?,
