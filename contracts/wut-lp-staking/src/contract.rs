...
pub fn instantiate(
    ...
    let config = Config {
        wut_token: deps.api.addr_canonicalize(&msg.wut_token)?,
        wut_lp_token: deps.api.addr_canonicalize(&msg.wut_lp_token)?,
...
pub fn receive_cw20(
    ...
    if config.wut_lp_token != deps.api.addr_canonicalize(info.sender.as_str())? {
